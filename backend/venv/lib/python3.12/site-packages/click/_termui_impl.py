...
class Editor:
    ...
    def edit_files(self, filenames: cabc.Iterable[str]) -> None:
        ...
        try:
            # Split the editor command to avoid shell=True
            editor_cmd = shlex.split(editor)
            # Use argument list instead of shell command string
            c = subprocess.Popen(
                args=editor_cmd + shlex.split(exc_filename), env=environ, shell=False
            )
            exit_code = c.wait()
            if exit_code != 0:
